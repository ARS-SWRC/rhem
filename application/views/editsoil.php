<!-- NOTE: This view has been renamed to editPaRfile -->
...
<head>
	...
	<script>
		 // set the editable functionality fo reach one of the values in the soil input file
		 $(document).ready(function() {
			$('span.editable133').editable133({
        		 indicator : 'Saving...', 
		 ...
		 function saveInputFile(saveoroverwrite)
		 {
			...
			// send a true flag to the editsoil controller in order for it to know to open the modified soil file instead of the current scenario's
			var modifiedsoilfileurl = $("#editPARfilelink").attr('href').replace("false","true");
			$("#editPARfilelink").attr('href', modifiedsoilfileurl);
			
			var inputhtml = $("#soilinputfile").html();
			
			// send the modified html to the server and save it under a generic soil file name for this user
			$.post("editsoil/save/",{soilinputhtml: inputhtml}, function(data){
				if(data.length > 0) {
					//alert("Soil File Saved Successfully.");
				}
			});
		 }
		 
		 /* This function will be used to hide the vegetation panels when 
		    the soil file has been manually modified*/
		 function hidePanels()
		 {
...
<p style="text-align:center;margin-top:30px;"> 
    ...
    <!--<input class="closeDOMWindow" type="button" value="Overwrite Input File" onClick="saveInputFile('overwrite');" alt="Saves the scenario using the current name. Overrides the soil file."/>-->
